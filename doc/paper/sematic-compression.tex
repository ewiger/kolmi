...
\subsection{Limits of Compression-Only Semantics}
...
Semantic preservation is defined behaviorally: if a model behaves similarly\footnote{judging by the output similarity} on both $x$ and $k(x)$, then the compression preserves semantics relative to that model. Formally, for a model $M$ and context $c$,
\[
f_M(x\mid c) \approx f_M(k(x)\mid c),
\]
where $f_M(\cdot\mid c)$ denotes model behavior under context $c$.
...
\textbf{Tau-equivalence:} We say $x \sim_{\tau, M} k(x)$ if $h_M(x)$ lies within the calibrated regime and $f_M(x\mid c) \approx f_M(k(x)\mid c)$ for relevant contexts $c$.
